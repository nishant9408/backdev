...
export class AuthController {
    ...
    public async registrationAndLogin(@Req() request: Request, @Res() response: Response): Promise<void> {
        ...
        let loginResp;
        try {
            loginResp = await this.authService.login(req, res);
        } catch (e) {
            // tslint:disable-next-line:no-console
            console.log('login err:', e);
        }
        try {
            await this.scheduleDailyScorePushes.execute({
                userId: user.id!,
                timezone: userData.timezone,
                notificationToken: userData.notificationToken,
            });
        ...
        try {
            await this.scheduleSilentDailyScorePushes.execute({
                userId: user.id!,
                timezone: userData.timezone,
                notificationToken: userData.notificationToken,
            });
            ...
            // tslint:disable-next-line:no-console
            console.log('silet error', e);
        }
        try {
            await this.repository.saveHealthProviderData(HealthProvider.fromObject({
                uId: user.id!,
                name: userData!.provider.name,
                timezone: userData.timezone,
                accessToken: userData.provider.accessToken,
