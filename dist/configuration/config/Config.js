...
exports.ALLOWED_NODE_ENV = void 0;
const assert = require("assert");
const DotEnv = require("dotenv");
const capitalize_1 = require("../../core/sharedKernel/capitalize");
exports.ALLOWED_NODE_ENV = [
    'local',
    'dev',
];
let config;
function configure() {
    ...
    assert(exports.ALLOWED_NODE_ENV.indexOf(nodeEnv) > -1);
    const path = `env/.env.${nodeEnv}`;
    DotEnv.config({ path });
    const envConfig = require(`./${(0, capitalize_1.capitalize)(nodeEnv)}Config`);
    return Object.assign({ apiPrefix: '/api', nodeEnv: process.env.NODE_ENV || 'local', port: process.env.PORT || '80', host: process.env.HOST || '0.0.0.0', cls: {
